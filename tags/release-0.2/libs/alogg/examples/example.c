...
void dom_do_it(char* file) {
	OGGFILE* ogg = open_ogg_file(file);

	if (ogg)
		play_ogg_file(ogg, BUFSZ, 255, 128);
	else 
		return;

  while (!keypressed()) {
    if ((ogg) && (poll_ogg_file(ogg) != ALOGG_OK)) {
			break;
    }
		usleep(200);
  }

  //while (poll_ogg_file(ogg) == ALOGG_OK) {
		// usleep(200);
	//}
